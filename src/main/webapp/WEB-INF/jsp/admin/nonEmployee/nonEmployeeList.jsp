...
<title>Hanon Systems Administrator Page <spring:message code="lnb.NAM.NEL" text="NonEmployee List" /></title>
<!-- (paging) -->
<style type="text/css">
...
$(document).ready(function(){
	...
	$("input[name='emp_status'").on("click",function(){ getNonEmployeeList(); });
	// Calendar 적용
    ...
    $('#contents input[name=content]').on('keypress',function(e){
        if (e.which == 13) { getNonEmployeeList(); }
    }); 
});

function init() {
	//left_menu 펼쳐 놓기
	$(".Non-Employee").addClass("on");
    $(".Non-Employee").find(">ul").show();
    $(".nonEmployeeList > a").addClass("highlight");
	// 사용자 목록 표시
	getNonEmployeeList();			// default: 기본 사용자 조회 없음
}

function getNonEmployeeList(){
	...
	// 사용자 조회 시작
	$.ajax({
		url: '/admin/nonEmployee/searchNonEmployeeList.do'
		, type: 'post'
		, dataType: 'xml'
		, data: params
...
function saveUserList(data) {
	...
	if(count > 0) {
		$('nonEmployeeUserInfoList', data).each(function() {
			cdsid = $('userid', this).text();
			managerId = $('immanagerid', this).text();
			name = $('cn', this).text();
			employeeStatus = $('imemployeestatus', this).text();
			employeType = $('destinationindicator', this).text();
			companyName = $('imcompanyname', this).text();
			activationDate = $('imactivationdate', this).text();
			expirationDate = $('imexpirationdate', this).text();
			var jsonString = '{"cdsid": "' + cdsid + '", "managerId": "' + managerId + '", "name": "' + name + '", "employeeStatus": "' + employeeStatus + '", "employeType": "' + employeType + '", "companyName": "' + companyName + '", "activationDate": "' + activationDate + '", "expirationDate": "' + expirationDate + '"} ';
...
function drawUserList(no) {
	...
	if($.isEmptyObject(userInfoList)) {
		$('#userListDiv').append("<tr><td colspan='10' class='center'><spring:message code='alert.result.1' text='List does not exist.' /></td></tr>");
		$('#paginationUser').text('');
	} else {
		...
		for(var i = pageObj.startRow - 1; i < pageObj.endRow; i++) {
			...
			employeType = userInfoList[i].employeType;
			...
			html = "<tr>"
				...
				+ "<td class='center'>" + managerId + "</td>"
				+ "<td class='left'>" + name + "</td>"
				+ "<td class='center'>" + activationDate + "</td>"
				+ "<td class='center'>" + expirationdate + "</td>"
				+ "<td class='center'>" + employeType + "</td>"
				+ "<td class='center'>" + companyName + "</td>"
				+ "<td class='center'><a class='btn_white' onclick='javascript:showUserModify(\""+ cdsid +"\");'><span class='clickable'>Modify</span></a></td>"
				+ "<td class='center'><a class='btn_white' onclick='javascript:userDelete(\""+ cdsid +"\");'><span class='clickable'>Delete</span></a></td>"
				+ "</tr>";
 ...
 * @since 2014-10-23
 */
function showUserDetail(guid) {
	$('.layer_popup').remove();
	
	$.ajax({
		url: '/admin/nonEmployee/view.do'
		, type: 'post'
		, dataType: 'xml'
		, data: 'userid=' + guid
 ...
 * @since 2018-07-11
 */
function showUserModify(userid) {
$('.layer_popup').remove();
	
	$.ajax({
		url: '/admin/nonEmployee/modify.do'
		, type: 'post'
		, dataType: 'json'
		, data: 'userid=' + userid
		, success: function(data, status, error) {
...
function userDelete(userid) {
	if(confirm("Delete this User?")){
		...
		$.ajax({
			url: '/admin/nonEmployee/deleteNonEmployeeInfo.do'
			, type: 'post'
			, dataType: 'json'
			, data: 'userid=' + userid
			, success: function(data, status, error) {
...
<div id="wrapper">
	...
	<div id="container">
		...
		<div id="contents">
			...
			<!-- (location) -->
			<div id="location">
				<h3><spring:message code="lnb.NAM.NEL" text="Non-Employee List" /></h3>
				
				<p class="page_navigation">
					<strong><spring:message code="lnb.NAM.NEL" text="Non-Employee List" /></strong>
					<span><spring:message code="lnb.NAM" text="Non-Employee Account Management" /></span>
					...
					<div id="searchbox">
						...
						<div class="regionbox">
							...
							<select name="title" title="Search Type" class="onclick" >
								...
								<option value="managerId"><spring:message code="label.managerId" text="Manager ID" /></option>
							...
							<a href="javascript:getNonEmployeeList()"><img src="/images/skin/btn/btn_allsearch.gif" alt="search" /></a>
						</div>
						<!-- //(searchbox) -->				
					</div>
				</div>
				...
				<table class="board_01" summary="User Quick Menu">
					...
					<thead>
						<tr>
							...
							<th scope="col"><a href="javascript:sortBy('managerId')"><spring:message code="page.managerId" text="Manager ID" /></a></th>
							<th scope="col"><a href="javascript:sortBy('name')"><spring:message code="label.name" text="Name" /></a></th>
							...
							<th scope="col"><a href="javascript:sortBy('employeeType')"><spring:message code="label.employeeType" text="Employee Type" /></a></th>
							<th scope="col"><a href="javascript:sortBy('companyName')"><spring:message code="label.companyName" text="Company Name" /></a></th>
							<th scope="col"><spring:message code="button.modify" text="Modify" /></th>
							<th scope="col"><spring:message code="button.delete" text="Delete" /></th>
						</tr>
					</thead>
