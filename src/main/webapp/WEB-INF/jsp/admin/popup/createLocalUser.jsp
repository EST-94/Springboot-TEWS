		...
		<div class="pop_contents">
			...
			<div class="clear">
				...
				<div class="regionbox">
					...
					<input type="text"  class="inputbox" style="padding-left:7px;" id="cdsid" name="cdsid" style="width: 165px;text-transform: uppercase;ime-mode:disabled !important;" />
					...
					<tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <textarea class="inputbox hasSearchbtn" readonly="readonly" name="roleName" id="RoleName" style="resize: none; text-indent: 0; padding-left: 4px;"></textarea>
                            </td>
                            <th scope="row" />
                            <td />
                        </tr>
			...
			<div class="center">
				<a class="btn_blue3" href="javascript:updateRegisterUser()"><span><spring:message code="button.save" text="Save" /></span></a>
				<a class="btn_gray4" href="javascript:closePopup('createLocalUser')"><span><spring:message code="button.close" text="Close" /></span></a>
			</div>

		</div>
	</div>

	<a href="javascript:closePopup('createLocalUser')" class="btn_pop_close"><img src="/images/skin/btn/btn_pop_close.gif" alt="close" /></a>
	</form>
</div>
...
$(document).ready(function(){
	...
	//검색 EnterKey 적용
	$('form[name=LocalUserForm] input[name=cdsid]').keydown(function(e){
		var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
		if (key == 13) { checkCdsid(); }
    });
	
	// CDSID를 수정하면, cdsid 체크 다시해야함
	$('form[name=LocalUserForm] input[name=cdsid]').change(function(e){
		$("#cdsidCheckResult").val("N");
		$('form[name=LocalUserForm] input[name=cdsid]').val($('form[name=LocalUserForm] input[name=cdsid]').val().toUpperCase());
...
function updateRegisterUser() {
	if(checkRequiredCreateUser()){
		$.ajax({
			url: '/imsso/localUser/register/update.do'
			, type: 'post'
			, dataType: 'xml'
			, data: $("#LocalUserForm").serialize()
			, success: function(data, status, error) {
				...
				if('1' == $('code', data).text()) {
					// success
					closePopup('createLocalUser');
				}
			}
			, error: function(data, status, error) {
				//console.log("Error with /imsso/localUser/user/update.do");
				//console.log(data);
			}
		});
	}
 ...
 * 필수 값 체크
 */
function checkRequiredCreateUser(){
	...
	if($("#firstName").val() == ""){
		...
		return false;
	} else if($("#cdsid").val() != "" && $("#checkCdsid").val() == "N") {
...
function checkCdsid(){
	if($("#cdsid").val() == ""){
		...
		ajaxJsonCall("/imsso/user/searchRegCdsId.do", params, function(data) {
			if(data["count"]>0){
...
function showProvisionSearch() {
	$.ajax({
		url: '/imsso/localUser/provision/view.do'
		, type: 'post'
		, dataType: 'xml'
		, success: function(data, status, error) {
