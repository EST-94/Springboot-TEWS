...
$(document).ready(function(){
	...
	// $("input[name='emp_status'").on("click",function(){ getUserList(); });
	// Calendar 적용
    ...
    $('#contents input[name=content]').on('keypress',function(e){
        if (e.which == 13) { getUserList(); }
    }); 
});

function init() {
	//left_menu 펼쳐 놓기
	$(".Self-Portal").addClass("on");
    $(".Self-Portal").find(">ul").show();
    $(".passwordReset > a").addClass("highlight");
	// 사용자 목록 표시
	//getUserList();			// default: 기본 사용자 조회 없음
	//$("#btn_upStatus").hide();
}
...
function getUserList() {
	...
	// 사용자 조회 시작
	$.ajax({
		url: '/admin/user/searchUserList.do'
		, type: 'post'
		, dataType: 'xml'
		, data: params
...
function saveUserList(data) {
	...
	if(count > 0) {
		$('userInfoList', data).each(function() {
			...
			itemObj = $.parseJSON('{"cdsid": "' + cdsid + '", "employeeType": "' + employeeType + '", "name": "' + name + '", "managerId": "' + managerId + '", "email": "' + email + '", "employeeStatus": "' + employeeStatus + '", "country": "' + country + '", "department": "' + department + '", "activationDate": "' + activationDate + '", "expirationDate": "' + expirationDate + '"} ');
...
function drawUserList(no) {
	...
	if($.isEmptyObject(userInfoList)) {
		$('#userListDiv').append("<tr><td colspan='7' class='center'><spring:message code='alert.result.1' text='List does not exist.' /></td></tr>");
		$('#paginationUser').text('');
	} else {
		...
		for(var i = pageObj.startRow - 1; i < pageObj.endRow; i++) {
			...
			html = "<tr>"
				...
				+ "<td class='center'><a class='btn_white' onclick='javascript:selectUserPasswordReset(\""+ cdsid +"\");'><span class='clickable'>Reset</span></a></td>"
 ...
 * @since 2018-07-11
 */
function selectUserPasswordReset(cdsid) {
	if(confirm("Do you want to reset " + cdsid + "'s password?")) {
		$('span.deem').css('display', 'block');
		$('div.loadingbar').css('display', 'block');
		
		$.ajax({
			url: '/admin/user/selectUserPasswordReset.do'
			, type: 'post'
			, dataType: 'json'
			, data: 'userid=' + cdsid
			, success: function(data, status, error) {
				...
				if(data.success == true) {
					var html = "New Password : " + data.value + "<br>Send Mail to User";
					...
					$('div.position_loading').children('dl').children('dt').html("Password reset successfull");
					$('div.position_loading').children('dl').children('dd').html(html);
					funcSendMail(cdsid, data.value);
				} else {
					$('div.position_loading').children('p').html("<a href='javascript:closeAlert();''><img align='right' style='padding-right: 20px;' src='/images/skin/btn/btn_pop_close.gif' alt=''/></a>");
					$('div.position_loading').children('dl').children('dt').html("Password reset failed");
...
function funcSendMail(cdsid, password) {
	$.ajax({
		url: '/admin/user/sendMailResetPassword.do'
		, type: 'post'
		, dataType: 'json'
		, data: {
			userid: cdsid,
...
<div id="wrapper">
	...
	<div id="container">
		<!-- (Contents lnb) -->
		<jsp:include page="/WEB-INF/jsp/include/lnb.jsp" />
		<!-- //(Contents lnb) -->
		...
		<div id="contents">
			...
			<!-- (location) -->
			<div id="location">
				<h3><spring:message code="lnb.SP.PR" text="Password Reset" /></h3>
				
				<p class="page_navigation">
					<strong><spring:message code="lnb.SP.PR" text="Password Reset" /></strong>
					<span><spring:message code="lnb.SP" text="Self-Poral" /></span>
					...
					<div id="searchbox">
						...
						<div class="regionbox">
							...
							<select name="title" title="Search Type" class="onclick" >
								...
								<option value="employeeType"><spring:message code="label.employeeType" text="Employee Type" /></option>
							...
							<a href="javascript:getUserList()"><img src="/images/skin/btn/btn_allsearch.gif" alt="search" /></a>
						</div>
						<!-- //(searchbox) -->				
					</div>					
				</div>
				...
				<table class="board_01" summary="User Quick Menu">
					...
					<thead>
						<tr>
							...
							<th scope="col"><a href="javascript:sortBy('employeeType')"><spring:message code="page.employeeType" text="Employee Type" /></a></th>
							<th scope="col"><a href="javascript:sortBy('name')"><spring:message code="label.nameEngLocal" text="Name" /></a></th>
							<th scope="col"><a href="javascript:sortBy('managerId')"><spring:message code="label.managerId" text="Manager ID" /></a></th>
							<th scope="col"><a href="javascript:sortBy('email')"><spring:message code="label.email" text="E-mail" /></a></th>
							...
							<th scope="col"><spring:message code="button.passwordReset" text="Password Reset" /></th>
						</tr>
					</thead>
