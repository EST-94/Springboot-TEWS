...
<title>Hanon Systems Administrator Page <spring:message code="lnb.SAM.SL" text="Supplier List" /></title>
<!-- (paging) -->
<style type="text/css">
...
$(document).ready(function(){
	...
	$("input[name='emp_status'").on("click",function(){ getSupplierList(); });
	// Calendar 적용
...
function init() {
    ...
    $(".supplierList > a").addClass("highlight");
	// 사용자 목록 표시
	getSupplierList();			// default: 기본 사용자 조회 없음
}
...
function getSupplierList(){
	...
	// 사용자 조회 시작
	$.ajax({
		url: '/admin/supplier/searchSupplierList.do'
		, type: 'post'
		, dataType: 'xml'
		, data: params
		...
		for(var i = pageObj.startRow - 1; i < pageObj.endRow; i++) {
			...
			html = "<tr>"
				...
				if(employeeStatus.toLowerCase() == 'inactive') {
					html += "<td class='center'><a class='btn_white' onclick='javascript:selectUserReactive(\""+ cdsid +"\");'><span class='clickable'>Reactive User</span></a></td>";	
				} else {
					html += "<td class='center'></td>";
				}
				if(employeeStatus.toLowerCase() == 'active'){
...
function selectUserRedButton(userid) {
	if(confirm(userid + " User Red button now?")) {
		...
		$.ajax({
			url: '/admin/supplier/selectUserRedButton.do'
			, type: 'post'
			, dataType: 'json'
			, data: 'userid=' + userid
			, success: function(data, status, error) {
...
function selectUserBlock(userid) {
	if(confirm(userid + " User Block now?")) {
		...
		$.ajax({
			url: '/admin/supplier/selectUserBlock.do'
			, type: 'post'
			, dataType: 'json'
			, data: 'userid=' + userid
			, success: function(data, status, error) {
 ...
 * @since 2018-07-11
 */
function selectUserReactive(userid) {
	if(confirm(userid + " User Reactive now?")) {
		$('span.deem').css('display', 'block');
		$('div.loadingbar').css('display', 'block');
		
		$.ajax({
			url: '/admin/supplier/selectUserReactive.do'
			, type: 'post'
			, dataType: 'json'
			, data: 'userid=' + userid
			, success: function(data, status, error) {
				...
				if(data.success == true) {
					$('div.position_loading').children('p').html("<a href='javascript:closeAlert();''><img align='right' style='padding-right: 20px;' src='/images/skin/btn/btn_pop_close.gif' alt=''/></a>");
					$('div.position_loading').children('dl').children('dt').html("Reactive User successfull");
					$('div.position_loading').children('dl').children('dd').html("");
				} else {
					$('div.position_loading').children('p').html("<a href='javascript:closeAlert();''><img align='right' style='padding-right: 20px;' src='/images/skin/btn/btn_pop_close.gif' alt=''/></a>");
					$('div.position_loading').children('dl').children('dt').html("Reactive User failed");
					$('div.position_loading').children('dl').children('dd').html(data.value);					
				}
			}, error: function(data, status, error) {
				//console.log("error");
...
<div id="wrapper">
	...
	<div id="container">
		...
		<div id="contents">
			...
			<!-- (location) -->
			<div id="location">
				<h3><spring:message code="lnb.SAM.SL" text="Supplier List" /></h3>
				
				<p class="page_navigation">
					<strong><spring:message code="lnb.SAM.SL" text="Supplier List" /></strong>
					<span><spring:message code="lnb.SAM" text="Supplier Account Management" /></span>
				...
				<div class="clear mgb_10">
					...
					<div id="searchbox">
						...
						<div class="regionbox">
							...
							<a href="javascript:getSupplierList()"><img src="/images/skin/btn/btn_allsearch.gif" alt="search" /></a>
						</div>
						<!-- //(searchbox) -->				
					</div>
				</div>
				...
				<table class="board_01" summary="User Quick Menu">
					...
					<thead>
						<tr>
							...
							<th scope="col"><spring:message code="button.oneButton" text="Red Button" /></th>
							<th scope="col"><spring:message code="button.reactiveUser" text="Reactive User" /></th>
